...
export function attachChatEvidenceToCase(
    ...
    labelOverride?: string  // e.g. 'Evidence File #2'
): void {
    const platformLabel = PLATFORM_CONFIG[scan.platform]?.fieldLabel || 'ID';
    const account = getConnectedAccounts().find(a => a.id === scan.accountId);
    const monitoredVia = account ? `${account.platform} account — ${account.handle}` : scan.platform;
    const fileLabel = labelOverride || `chat-transcript`;
    const filename = `${fileLabel}-${scan.platform.replace(/\s+/g, '_')}-${scan.contactHandle.replace(/[^a-zA-Z0-9_+@.-]/g, '_')}-${Date.now()}.txt`;

    // Build a human-readable transcript with full contact identity header
    const lines: string[] = [
        ...
        `  ${platformLabel.padEnd(13)}: ${scan.contactHandle}`,
        `  Platform     : ${scan.platform}`,
        `  Monitored Via: ${monitoredVia}`,
        ``,
        `SCAN DETAILS`,
        `${'-'.repeat(70)}`,
        `  Case ID      : ${caseId}`,
        ...
        `  AI Source    : ${scan.aiAnalysisSource}`,
        ``,
        `RISK SUMMARY`,
        `${'-'.repeat(70)}`,
        scan.riskSummary,
        ...
        `${'-'.repeat(70)}`,
        ...scan.messages.map(m =>
            `[${new Date(m.timestamp).toLocaleString()}] ${m.direction === 'outgoing' ? 'ME (VICTIM)' : `${m.sender.toUpperCase()} [${scan.contactHandle}]`}: ${m.content}`
        ),
        ``,
        `${'='.repeat(70)}`,
        `Generated by Evidence Vault Social Monitor · ${new Date().toUTCString()}`,
    ];
    ...
    localStorage.setItem(storageKey, JSON.stringify({
        name: filename, type: 'text/plain', size: bytes.length,
        data: dataUrl, uploadedAt: new Date().toISOString(),
    }));
...
// ─── Attach ALL evidence for a contact (by handle) to a case ─────────────────
// Finds every scan matching the same contactHandle and attaches each as a
// separate evidence file. Also prepends a Contact Profile Summary.
export function attachAllContactEvidenceToCase(
    caseId: string,
    primaryScan: ScannedConversation,
    uploaderEmail: string
): void {
    const allScans = getAllScans();

    // Collect all scans for this contact (same handle OR same name on same platform)
    const relatedScans = allScans.filter(s =>
        s.contactHandle === primaryScan.contactHandle ||
        (s.contactName === primaryScan.contactName && s.platform === primaryScan.platform)
    );

    const scanSet = relatedScans.length > 0 ? relatedScans : [primaryScan];
    const platformLabel = PLATFORM_CONFIG[primaryScan.platform]?.fieldLabel || 'ID';

    // 1. Create a Contact Profile Summary as the FIRST evidence file
    const profileLines: string[] = [
        `Evidence Vault — Contact Threat Profile`,
        `${'='.repeat(70)}`,
        ``,
        `SUBJECT IDENTITY`,
        `${'-'.repeat(70)}`,
        `  Name         : ${primaryScan.contactName}`,
        `  ${platformLabel.padEnd(13)}: ${primaryScan.contactHandle}`,
        `  Primary Plat : ${primaryScan.platform}`,
        `  Max Risk     : ${Math.max(...scanSet.map(s => s.riskLevel))}/10 — ${getRiskLabel(Math.max(...scanSet.map(s => s.riskLevel)) as RiskLevel)}`,
        `  Evidence Files: ${scanSet.length}`,
        `  Case ID      : ${caseId}`,
        `  Report Date  : ${new Date().toUTCString()}`,
        ``,
        `THREAT SUMMARY ACROSS ALL CONVERSATIONS`,
        `${'-'.repeat(70)}`,
        ...scanSet.map((s, i) =>
            `  [${i + 1}] ${s.platform} — Risk ${s.riskLevel}/10 — Scanned ${new Date(s.scannedAt).toLocaleString()}\n       ${s.riskSummary}`
        ),
        ``,
        `ALL DETECTED THREATS`,
        `${'-'.repeat(70)}`,
        ...Array.from(new Set(scanSet.flatMap(s => s.detectedThreats))).map(t => `  • ${t}`),
        ``,
        `CONSOLIDATED RECOMMENDATIONS`,
        `${'-'.repeat(70)}`,
        ...Array.from(new Set(scanSet.flatMap(s => s.recommendations))).map(r => `  → ${r}`),
        ``,
        `EVIDENCE FILES ATTACHED`,
        `${'-'.repeat(70)}`,
        ...scanSet.map((s, i) => `  [${i + 1}] chat-transcript-${s.platform.replace(/\s+/g, '_')}-${s.contactHandle.replace(/[^a-zA-Z0-9_+@.-]/g, '_')}-*.txt`),
        ``,
        `${'='.repeat(70)}`,
        `Generated by Evidence Vault Social Monitor · ${new Date().toUTCString()}`,
    ];

    const profileContent = profileLines.join('\n');
    const profileBytes = new TextEncoder().encode(profileContent);
    const profileKey = `ev_file_${caseId}_profile_${Date.now()}`;
    const profileB64 = btoa(String.fromCharCode(...profileBytes));
    localStorage.setItem(profileKey, JSON.stringify({
        name: `CONTACT_PROFILE-${primaryScan.contactHandle.replace(/[^a-zA-Z0-9_+@.-]/g, '_')}-${caseId}.txt`,
        type: 'text/plain', size: profileBytes.length,
        data: `data:text/plain;base64,${profileB64}`,
        uploadedAt: new Date().toISOString(),
    }));
    let profileHash = 0;
    for (let i = 0; i < profileContent.length; i++) {
        profileHash = ((profileHash << 5) - profileHash) + profileContent.charCodeAt(i);
        profileHash |= 0;
    }
    saveEvidenceToCase(caseId, {
        id: `contact-profile-${primaryScan.id}-${Date.now()}`,
        file_name: `CONTACT_PROFILE-${primaryScan.contactHandle.replace(/[^a-zA-Z0-9_+@.-]/g, '_')}-${caseId}.txt`,
        file_type: 'text/plain',
        file_size: profileBytes.length,
        client_sha256: Math.abs(profileHash).toString(16).padStart(16, '0'),
        server_sha256: Math.abs(profileHash).toString(16).padStart(16, '0'),
        uploaded_by: uploaderEmail,
        upload_timestamp: new Date().toISOString(),
        integrity_status: 'VERIFIED',
    ...
    // 2. Attach each conversation as a numbered evidence file
    scanSet.forEach((s, i) => {
        // Small delay in key to avoid collisions
        setTimeout(() => {
            attachChatEvidenceToCase(caseId, s, uploaderEmail, `evidence-${i + 1}-of-${scanSet.length}`);
        }, i * 5);
    });
}
